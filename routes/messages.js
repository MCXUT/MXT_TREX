...
router.post("/start_message", (req, res) => {
    Partner.findOne({email: req.body.send_to}, function(err, foundPartner) {
        if (err || !foundPartner) {
            req.flash("error_messages", "파트너를 찾을수 없습니다. 다시 시도해주세요.");
        ...
        newMessage.save((err) => {
              if (err) {
                  console.log(err);
                  req.flash("error_messages", "메세지를 보낼수 없습니다. 다시 시도해주세요.");
                  return res.redirect("/user_profile");
              }
    ...
    // req.flash("error_messages", "메세지를 보내는데 문제가 발생 했습니다. 다시 시도해주세요.");
    // return res.redirect("/user_profile");
});
...
router.get("/message_room/:messageID", (req, res) => {
    if (req.user) {
        Message.findById(req.params.messageID, (err, foundMessage) => {
            if (err || !foundMessage) {
                req.flash("error_messages", "메세지를 찾을수 없습니다. 다시 시도해주세요.");
                return res.redirect("/user_profile");
            }
